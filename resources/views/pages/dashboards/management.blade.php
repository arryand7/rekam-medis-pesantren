<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <div class="border-b border-[var(--border)] pb-5 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-[var(--foreground)] flex items-center gap-3">
                    <span class="p-2.5 bg-[var(--info-bg)] text-[var(--info-text)] border border-[var(--border)] rounded-xl shadow-xs">
                ...
                <p class="mt-1.5 text-sm ui-text-muted">
                    Statistik agregat operasional kesehatan, tren kunjungan, evaluasi rujukan, dan kepatuhan kontrol santri.
                </p>
            </div>

            <!-- Date Range Filter Form -->
            <form method="GET" action="{{ route('dashboards.management') }}" class="ui-card flex flex-wrap items-center gap-2 p-2 rounded-2xl shadow-xs">
                <div class="flex items-center gap-1">
                    <a href="{{ route('dashboards.management', ['preset' => 'today']) }}" class="ui-filter-chip px-2.5 py-1.5 text-xs font-medium rounded-xl transition {{ $preset === 'today' ? 'ui-filter-chip-active' : '' }}">Hari Ini</a>
                    <a href="{{ route('dashboards.management', ['preset' => '7_days']) }}" class="ui-filter-chip px-2.5 py-1.5 text-xs font-medium rounded-xl transition {{ $preset === '7_days' ? 'ui-filter-chip-active' : '' }}">7 Hari</a>
                    <a href="{{ route('dashboards.management', ['preset' => '30_days']) }}" class="ui-filter-chip px-2.5 py-1.5 text-xs font-medium rounded-xl transition {{ $preset === '30_days' ? 'ui-filter-chip-active' : '' }}">30 Hari</a>
                    <a href="{{ route('dashboards.management', ['preset' => 'this_month']) }}" class="ui-filter-chip px-2.5 py-1.5 text-xs font-medium rounded-xl transition {{ $preset === 'this_month' ? 'ui-filter-chip-active' : '' }}">Bulan Ini</a>
                </div>
                ...
                <div class="flex items-center gap-1.5 text-xs">
                    <input type="date" name="from" value="{{ $fromInput ?? $metrics['period']['start_raw'] }}" aria-label="Tanggal mulai" class="ui-form-control px-2 py-1 border rounded-lg text-xs focus:ring-1 focus:ring-[var(--focus-ring)]">
                    <span class="ui-text-muted">&ndash;</span>
                    <input type="date" name="to" value="{{ $toInput ?? $metrics['period']['end_raw'] }}" aria-label="Tanggal akhir" class="ui-form-control px-2 py-1 border rounded-lg text-xs focus:ring-1 focus:ring-[var(--focus-ring)]">
                    <button type="submit" class="px-3 py-1 bg-[var(--action-bg)] hover:bg-[var(--action-hover)] text-[var(--action-text)] rounded-lg font-semibold transition">
                        Terapkan
                    </button>
                </div>
            </form>
        ...
        <!-- Period Banner -->
        <div class="ui-banner-info flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 p-3.5 rounded-2xl text-xs">
            <div class="flex items-center gap-2">
                <svg class="w-4 h-4 text-[var(--info)] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 002 2z"/></svg>
            ...
            <a href="{{ route('reports.index') }}" class="underline font-bold hover:text-[var(--foreground)]">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            ...
            <!-- Total Visits -->
            <div class="ui-card p-5 rounded-2xl shadow-xs flex flex-col justify-between">
                <div>
                    <span class="text-xs font-semibold uppercase tracking-wider ui-text-secondary">Total Kunjungan Medis</span>
                ...
                <div class="mt-3 pt-3 border-t border-[var(--border)] text-xs">
                    ...
                    @else
                        <span class="ui-text-muted italic">{{ $metrics['visits_comparison']['label'] }}</span>
                    @endif
                </div>
            </div>
            ...
            <div class="ui-card p-5 rounded-2xl shadow-xs flex flex-col justify-between">
                <div>
                    <span class="text-xs font-semibold uppercase tracking-wider ui-text-secondary">Pasien Unik Dilayani</span>
                    <div class="text-3xl font-extrabold text-indigo-600 dark:text-indigo-400 mt-2">{{ $metrics['unique_patients'] }}</div>
                </div>
                <div class="mt-3 pt-3 border-t border-[var(--border)] text-xs ui-text-muted">
            ...
            <div class="ui-card p-5 rounded-2xl shadow-xs flex flex-col justify-between">
                <div>
                    <span class="text-xs font-semibold uppercase tracking-wider ui-text-secondary">Kepatuhan Kontrol / Follow-Up</span>
                    <div class="text-3xl font-extrabold text-emerald-600 dark:text-emerald-400 mt-2">
                        ...
                        @else
                            <span class="text-lg font-semibold ui-text-muted">Belum ada data</span>
                        @endif
                    </div>
                </div>
                <div class="mt-3 pt-3 border-t border-[var(--border)] text-xs ui-text-muted">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 ui-card rounded-2xl shadow-xs p-5 space-y-4">
                <div class="flex items-center justify-between">
                    <div>
                        <h2 class="text-sm font-bold text-[var(--foreground)]">Tren Volume Kunjungan & Observasi</h2>
                        <p class="text-xs ui-text-muted">Distribusi volume aktivitas pelayanan poskestren sepanjang periode</p>
                    </div>
                    <div class="flex items-center gap-3 text-xs">
                        <span class="ui-chart-label flex items-center gap-1.5">
                    ...
                    <div class="flex items-end gap-1.5 h-44 border-b border-[var(--border)] pb-1 overflow-x-auto">
                        @foreach($metrics['daily_trends'] as $day)
                            ...
                            <div class="flex-1 min-w-[28px] flex flex-col items-center gap-1 group relative">
                                ...
                                <!-- X-axis label -->
                                <span class="ui-chart-label text-[9px] truncate w-full text-center mt-1">{{ $day['label'] }}</span>
                            </div>
                        @endforeach
                    </div>
                </div>
                ...
                <details class="text-xs ui-text-muted cursor-pointer">
            ...
            <div class="ui-card rounded-2xl shadow-xs p-5 space-y-4 flex flex-col justify-between">
                <div>
                    <h2 class="text-sm font-bold text-[var(--foreground)]">Status Kesehatan Stok Farmasi</h2>
                    <p class="text-xs ui-text-muted">Kondisi kelayakan batch obat aktif saat ini</p>
                </div>

                <div class="space-y-3 my-auto">
                    <div class="ui-banner-success flex items-center justify-between p-3 rounded-xl">
                        <span class="text-xs font-semibold">Batch Aktif & Aman</span>
                        <span class="text-sm font-bold">{{ $metrics['batch_health']['active'] }}</span>
                    </div>
                    <div class="ui-banner-warning flex items-center justify-between p-3 rounded-xl">
                        <span class="text-xs font-semibold">Near-Expiry (&le; 30 Hari)</span>
                        <span class="text-sm font-bold">{{ $metrics['batch_health']['near_expiry'] }}</span>
                    </div>
                    <div class="ui-banner-danger flex items-center justify-between p-3 rounded-xl">
                        <span class="text-xs font-semibold">Kedaluwarsa</span>
                        <span class="text-sm font-bold">{{ $metrics['batch_health']['expired'] }}</span>
                    </div>
                ...
                <div class="text-xs ui-text-muted pt-2 border-t border-[var(--border)] flex justify-between items-center">
        ...
        <div class="ui-surface-subtle p-4 border rounded-2xl text-xs ui-text-muted flex items-center gap-3">
            <svg class="w-5 h-5 text-[var(--info)] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
