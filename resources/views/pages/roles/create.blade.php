    ...
    <div class="space-y-6" x-data="{
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-[var(--surface)] p-6 rounded-2xl border border-[var(--border)] shadow-xs">
            <div>
                ...
                <h1 class="text-2xl font-bold text-[var(--foreground)] tracking-tight">Buat Role & Paket Hak Akses Baru</h1>
                <p class="text-sm text-[var(--foreground-muted)] mt-1">
                    Tentukan identifier unik, nama tampilan, dan centang hak akses (permissions) yang diizinkan untuk peran ini.
                </p>
            </div>
        ...
        <form method="POST" action="{{ route('roles.store') }}" class="space-y-6">
            ...
            <div class="bg-[var(--surface)] p-6 rounded-2xl border border-[var(--border)] shadow-xs space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input
                            ...
                            id="name"
                            value="{{ old('name') }}"
                            placeholder="contoh: staf_laboratorium atau kasir_poskestren"
                            required
                            class="w-full text-xs font-mono p-2.5 rounded-xl border border-[var(--border)] bg-[var(--surface)] text-[var(--foreground)] focus:ring-2 focus:ring-blue-500">
                        <p class="text-[11px] text-[var(--foreground-muted)] mt-1">Gunakan huruf kecil, angka, dan garis bawah (_). Tidak dapat diubah setelah role dibuat.</p>
                    </div>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            placeholder="contoh: Staf Laboratorium Poskestren"
                            required
                        ...
                        <p class="text-[11px] text-[var(--foreground-muted)] mt-1">Nama resmi yang tampil pada antarmuka aplikasi.</p>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea
                        ...
                        id="description"
                        rows="2"
                        placeholder="Uraian ringkas kewenangan operasional dari role ini..."
                        class="w-full text-xs p-2.5 rounded-xl border border-[var(--border)] bg-[var(--surface)] text-[var(--foreground)] focus:ring-2 focus:ring-blue-500">{{ old('description') }}</textarea>
                </div>
            </div>
            ...
            <div class="bg-[var(--surface)] p-6 rounded-2xl border border-[var(--border)] shadow-xs space-y-6">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[var(--border)] pb-4">
                    <div>
                        ...
                        <p class="text-xs text-[var(--foreground-muted)] mt-0.5">Pilih permission yang akan diberikan secara otomatis kepada setiap pemegang role ini.</p>
                    </div>
                                        ...
                                        <div class="min-w-0 flex-1">
                                            ...
                                            @if($isDisabled)
                                                <div class="text-[9px] text-amber-600 dark:text-amber-400 mt-0.5">
                                                    * Hanya Super Admin yang dapat memberikan hak ini.
                                                </div>
                                            @endif
                                        </div>
                                    </label>
            ...
            <div class="flex items-center justify-end gap-3 pt-2">
                <a href="{{ route('roles.index') }}" class="px-5 py-2.5 rounded-xl border border-[var(--border)] bg-[var(--surface)] text-[var(--foreground-muted)] hover:text-[var(--foreground)] text-xs font-semibold transition-colors">
                    Batal
                </a>
