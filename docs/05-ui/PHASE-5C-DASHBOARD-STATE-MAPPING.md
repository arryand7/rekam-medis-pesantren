...
| **Stok Menipis (Low Stock)** | `Medicine` | `SUM(batches.current_quantity) <= $threshold` | Default teknis 10 unit (`[PERLU DIKONFIRMASI]`) |
| **Mutasi Buku Besar Hari Ini** | `MedicineStockMovement` | `created_at >= NOW()->startOfDay()` | Seluruh mutasi masuk, keluar, penyesuaian, dan reversal |
...
|---|---|---|---|
| **Total Kunjungan Medis** | `MedicalVisit` | `COUNT(id) WHERE created_at BETWEEN from AND to` | Komparasi terhadap durasi hari periode sebelumnya |
| **Pasien Unik Dilayani** | `MedicalVisit` | `COUNT(DISTINCT patient_id) WHERE created_at BETWEEN from AND to` | Bebas identitas individu |
...
| **Tingkat Kepatuhan Kontrol** | `VisitFollowUpPlan` | `(COUNT(status='completed') / COUNT(*)) * 100` | 100% jika denominator = 0 |
| **Volume Mutasi Farmasi** | `MedicineStockMovement` | `COUNT(id) WHERE created_at BETWEEN from AND to` | Agregat murni |
