...
- **Definisi**: Jumlah batch obat dengan tanggal kedaluwarsa dalam rentang jendela peringatan (default 30 hari ke depan) dan masih memiliki sisa stok.
- **Formula**: `medicine_batches.expiry_date BETWEEN CURRENT_DATE AND (CURRENT_DATE + interval warning_days) AND current_quantity > 0`.
- **Status Kebijakan**: Default teknis 30 hari (`[PERLU DIKONFIRMASI]`).
...
- **Definisi**: Jumlah item master obat aktif yang total akumulasi kuantitas seluruh batchnya kurang dari atau sama dengan ambang batas.
- **Ambang Batas**: Default teknis `<= 10` unit (`[PERLU DIKONFIRMASI]`).
...
- **Formula**: `COUNT(id) FROM medical_visits WHERE created_at BETWEEN from AND to`.
...
- **Formula**: `COUNT(DISTINCT patient_id) FROM medical_visits WHERE created_at BETWEEN from AND to`.
...
- **Null Handling**: Jika Denominator = 0, kembalikan `100.0%` (atau `0.0%` jika tidak ada rencana).

### D. Comparison Calculation (Perhitungan Komparasi Periode)
- **Formula**: `((Nilai_Sekarang - Nilai_Lalu) / Nilai_Lalu) * 100`.
- **Zero Denominator Handling**: Jika `Nilai_Lalu = 0`, sistem **DILARANG** menampilkan lonjakan persentase buatan (misal +999%). Sistem menampilkan string: `Tidak tersedia pembanding`.
