...
title: "Phase 5C Metric and KPI Definitions"
status: active
owner: "Antigravity AI / Ryand Arifriantoni"
last_updated: 2026-08-14
---

# Phase 5C Metric and KPI Definitions
...
Empat status stok di bawah ini dijamin **100% Saling Lepas (Mutually Exclusive)**:

### A. Expired Batches (Batch Kedaluwarsa)
- **Definisi**: Jumlah batch obat dengan tanggal kedaluwarsa lebih kecil dari hari ini dan masih memiliki sisa stok (`current_quantity > 0`).
- **Field Sumber**: `medicine_batches.expiry_date < CURRENT_DATE AND medicine_batches.current_quantity > 0` (`MedicineBatch::scopeExpired()`).
...
- **Formula**: `medicine_batches.expiry_date BETWEEN CURRENT_DATE AND (CURRENT_DATE + interval config('pharmacy.expiry_warning_days')) AND current_quantity > 0` (`MedicineBatch::scopeNearExpiry()`).
- **Status Kebijakan**: Default teknis 30 hari (`[PERLU DIKONFIRMASI DENGAN SOP FARMASI RESMI]`).

### C. Normal / Safe Batches (Batch Aman & Aktif)
- **Definisi**: Jumlah batch obat aktif dengan tanggal kedaluwarsa melampaui batas jendela peringatan dan memiliki sisa stok.
- **Formula**: `medicine_batches.expiry_date > (CURRENT_DATE + interval config('pharmacy.expiry_warning_days')) AND current_quantity > 0` (`MedicineBatch::scopeNormal()`).

### D. Depleted Batches / Out of Stock (Batch Habis)
- **Definisi**: Jumlah batch obat aktif yang memiliki sisa stok sama dengan 0.
- **Field Sumber**: `medicine_batches.current_quantity <= 0` (`MedicineBatch::scopeDepleted()`).

### E. Low Stock Medicines (Stok Obat Menipis)
...
1. `start_date` dan `end_date`: Membatasi rentang pencatatan secara inklusif (untuk sensus historis).
...
## 5. Pembedaan Semantik: Laporan Stok vs Mutasi Buku Besar

- **Laporan Stok Farmasi (`pharmacy_stock`)**: Merupakan **Snapshot Keadaan Stok Terkini (*Current Inventory Snapshot*)** pada saat pelaporan dibuat. Laporan ini tidak menggunakan rentang tanggal historis, melainkan menyajikan kuantitas fisik dan status kedaluwarsa riil batch saat ini.
- **Mutasi Buku Besar Farmasi (`stock_movements`)**: Merupakan **Log Transaksional Append-Only (*Historical Ledger*)** yang mencatat setiap perpindahan fisik obat (penerimaan, pemberian/resep, penyesuaian opname, dan pemusnahan) lengkap dengan timestamp kejadian.
