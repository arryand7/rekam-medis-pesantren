...
- [ ] Client ID/secret tersimpan di secret manager; callback persis sama dengan `GATE_REDIRECT_URI` HTTPS.
- [ ] Scope minimum, `GATE_APP_CODE`, entitlement status dan expiry/refresh semantics dikonfirmasi.
...
Aktifkan `GATE_SSO_ENABLED`, `GATE_SYNC_APPLY_ENABLED`, dan webhook secara terpisah setelah setiap tahap lulus. Phase 5D tidak melakukan panggilan Gate nyata.
